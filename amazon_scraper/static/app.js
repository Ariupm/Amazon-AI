...
function renderBatch(batch){
  summaryBox.innerHTML=`<div><b>本批次结果</b><span>${batch.total} 个输入 · ${batch.succeeded} 成功 · ${batch.failed} 失败</span></div><div><button onclick="exportExcel(this)">导出 Excel（含图片）</button><button onclick="exportCSV()">导出 CSV</button><button onclick="exportJSON()">导出 JSON</button></div>`;
...
function rows(){return latestBatch.items.flatMap(item=>item.success?item.result.variants.map(v=>({input:item.requested_asin,parent:item.result.parent_asin||"",is_parent:item.result.is_parent_request?"是":"否",suspected_main:v.is_suspected_main?"是":"否",main_confidence:v.is_suspected_main?(item.result.suspected_main_confidence||""):"",main_reason:v.main_reason||"",asin:v.asin,title:v.title||"",price:v.price||"",typical_price:v.list_price||"",discount:v.discount||"",color:v.color||"",size:v.size||"",monthly_sales_signal:v.recent_sales_signal||"",monthly_sales_estimate:v.monthly_sales_estimate??"",rating:v.rating??"",rating_count:v.rating_count??"",availability:v.availability||"",url:v.url,quality:v.data_quality})):[{input:item.requested_asin,error:item.error}])}
...
async function exportExcel(button){const original=button.textContent;button.disabled=true;button.textContent="正在嵌入图片…";try{const response=await fetch("/api/export/xlsx",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(latestBatch)});if(!response.ok){const error=await response.json();throw new Error(error.detail||"Excel 导出失败")}const url=URL.createObjectURL(await response.blob()),a=document.createElement("a");a.href=url;a.download="amazon-products-with-images.xlsx";a.click();URL.revokeObjectURL(url)}catch(error){alert(error.message)}finally{button.disabled=false;button.textContent=original}}
